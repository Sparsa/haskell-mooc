...
updateAt :: Int -> a -> [a] -> [a]
updateAt i x xs = take i xs ++ [x] ++ takeFinal (length xs - (i + 1)) xs
...
substring i j s = take (j - i) (takeFinal (length s - i) s)
...
isPalindrome str
  | str == "" = True
  | length str == 1 = True
  | (take 1 str) == (takeFinal 1 str) = isPalindrome (substring 1 (length str - 1) str)
  | otherwise = False
...
palindromify s
  | isPalindrome s = s
  | otherwise = palindromify (take ((length s) - 2) (tail s))
...
greet first last = "Hello, " ++ first ++ a ++ "!"
  where
    a = case last of
      Just b -> " " ++ b
      Nothing -> ""
...
safeIndex xs i
  ...
  | otherwise = Just (head (takeFinal (length xs - i) xs))
